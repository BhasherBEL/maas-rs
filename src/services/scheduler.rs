...
use crate::services::build::{apply_routing_defaults, build_gtfs_phase, finalize_contraction};
use crate::services::fingerprint::{graph_fingerprint, osm_fingerprint};
...
pub fn spawn(graph: SharedGraph, config: Arc<Config>) {
    ...
    tokio::spawn(async move {
        run_loop(graph, config, schedule, cache_dir).await;
    });
}

async fn run_loop(graph: SharedGraph, config: Arc<Config>, schedule: Schedule, cache_dir: String) {
    // Startup catch-up: refresh once if a scheduled tick elapsed while down.
...
fn run_update_cycle(graph: &SharedGraph, config: &Config, cache_dir: &str) -> Result<bool, String> {
    ...
    // The GTFS phase reuses osm.bin, so it must still match the OSM inputs+params.
    let osm_fp = osm_fingerprint(config, cache_dir);
    let osm = load_osm_graph(&config.build.osm_output, &osm_fp).map_err(|e| e.0)?;
    ...
    // Drop interior arrays before persisting/swapping, else the refresh reverts the
    // memory win and saves a bloated graph.bin.
    finalize_contraction(&mut new_graph)?;

    // Stamp refreshed content into graph.bin's fingerprint so the next cold start
    // reuses it instead of rebuilding.
    let graph_fp = graph_fingerprint(config, cache_dir);
    save_graph_with_rollback(&new_graph, &graph_fp, &config.build.output)?;
...
/// Prepends a "0" seconds field to a 5-field expression for the `cron` crate.
